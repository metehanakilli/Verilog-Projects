...
// 20 BIT SHIFT REGISTER
	...
	always @(posedge clk_out or negedge rst_n) begin : shift_reg        //WRITING SYNCHRONIZED BUTTON INPUT IN "samples"
		if(!rst_n) begin          					               
			samples <= 20'b0;					
		end else begin
...
//IF THE NUMBER OF 1'S IS MORE THAN SUFFICIENT_NUMBER_OF_ONES, btn_out BECOMES 1
	always @(posedge clk or negedge rst_n) begin : debounce_input
		if(!rst_n) begin 
			btn_out <= 1'b0;
		end else begin
